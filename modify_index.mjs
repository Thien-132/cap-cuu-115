...
let indexStr = fs.readFileSync('d:/cap-cuu-115/src/routes/index.tsx', 'utf8');
...
// Insert the imports at line 34 (before heroImg import)
lines.splice(33, 0, ...imports.split('\n'));
...
// Add NAV prop to Navbar and Footer in the Index component
let indexComponentStart = findFunc('Index', currentLines);
for(let i=indexComponentStart; i<currentLines.length; i++) {
  if (currentLines[i].includes('<Navbar onOpenBooking')) {
    currentLines[i] = currentLines[i].replace('<Navbar ', '<Navbar navItems={NAV} ');
  }
...
fs.writeFileSync('d:/cap-cuu-115/src/routes/index.tsx', currentLines.join('\n'));
console.log('Modified index.tsx successfully.');

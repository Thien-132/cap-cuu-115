      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <a href="#contact" className="inline-flex items-center gap-2 rounded-full gradient-sky px-4 py-2 text-sm font-semibold text-primary-foreground shadow-soft hover:opacity-95 transition">
              Đặt lịch ngay
            </a>
          </div>
      ...
      {open && (
        <div className="md:hidden border-t border-border bg-background/95 backdrop-blur-md animate-fade-in">
          <div className="px-4 py-4 flex flex-col gap-2">
            ...
            <a href="#contact" onClick={() => setOpen(false)} className="inline-flex items-center justify-center gap-2 rounded-full gradient-sky px-4 py-3 text-sm font-semibold text-primary-foreground">
              Đặt lịch
            </a>
          </div>
        </div>
  ...
  return (
    <section id="home" className="relative isolate overflow-hidden pt-24 pb-16 sm:pt-32 sm:pb-24">
      <div className="absolute inset-0 -z-10">
        <img
          src="https://images.unsplash.com/photo-1576091160550-2173ff9e5ee4?auto=format&fit=crop&q=80"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl animate-fade-in">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            ...
            <a href="#contact" className="inline-flex items-center justify-center gap-2 rounded-full gradient-sky px-6 py-3.5 text-base font-semibold text-primary-foreground shadow-soft hover:scale-[1.02] transition-transform">
...
function WhyUs() {
  const items = [
    { icon: HeartHandshake, title: "Chăm sóc cá nhân hóa", desc: "Mỗi bệnh nhân đều có phác đồ chăm sóc riêng biệt phù hợp với tình trạng sức khỏe." },
    { icon: UserCheck, title: "Đội ngũ chuyên nghiệp", desc: "100% điều dưỡng viên có chứng chỉ hành nghề, được đào tạo bài bản và giàu kinh nghiệm." },
    { icon: Clock, title: "Tiết kiệm thời gian", desc: "Không cần xếp hàng chờ đợi ở bệnh viện, dịch vụ đến tận nơi giúp bạn an tâm nghỉ ngơi." },
    { icon: ShieldCheck, title: "Đảm bảo an toàn", desc: "Tuân thủ nghiêm ngặt quy trình vô khuẩn, kiểm soát nhiễm khuẩn theo tiêu chuẩn của Bộ Y tế." },
    { icon: Activity, title: "Cập nhật liên tục", desc: "Theo dõi sát sao sinh hiệu và báo cáo tình trạng bệnh nhân thường xuyên cho bác sĩ/gia đình." },
    { icon: Pill, title: "Phục vụ mọi lúc", desc: "Hỗ trợ linh hoạt theo giờ, theo ngày hoặc chăm sóc dài hạn 24/7 tùy nhu cầu của gia đình." },
  ];
  return (
    <section className="py-20 sm:py-24 bg-secondary/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((it) => (
            <div key={it.title} className="group rounded-2xl border border-border bg-card p-6 shadow-card hover:shadow-soft hover:-translate-y-1 transition-all duration-300">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((s, i) => (
            <article key={i} className="group flex flex-col rounded-2xl overflow-hidden border border-border bg-card p-6 shadow-card hover:shadow-soft hover:-translate-y-1 transition-all duration-300">
              ...
              <div className="mt-6 flex-1 flex items-end gap-5">
                <a href="https://zalo.me/0915205115" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:gap-2.5 transition-all">
                  Tư vấn ngay <ArrowRight className="h-4 w-4" />
                </a>
                <a href="#contact" className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:gap-2.5 transition-all">
...
function HowItWorks() {
  const steps = [
    { icon: PhoneCall, title: "Liên hệ & Tư vấn", desc: "Gọi hotline hoặc để lại thông tin. Chúng tôi sẽ tư vấn dịch vụ phù hợp nhất." },
    { icon: ClipboardList, title: "Đánh giá tình trạng", desc: "Đội ngũ y tế trao đổi chi tiết về bệnh lý, hồ sơ sức khỏe và nhu cầu chăm sóc." },
    { icon: Ambulance, title: "Đến tận nhà", desc: "Điều dưỡng viên mang theo trang thiết bị y tế đến đúng giờ, đúng địa điểm." },
    { icon: CheckCircle2, title: "Tiến hành chăm sóc", desc: "Thực hiện thủ thuật chuyên môn, theo dõi diễn biến và báo cáo cho gia đình." },
  ];
  return (
    <section id="howitworks" className="py-20 sm:py-24 bg-gradient-to-b from-secondary/40 to-background">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-5 relative">
          {steps.map((s, i) => (
            <div key={i} className="relative rounded-2xl border border-border bg-card p-6 shadow-card hover:shadow-soft transition-all">
          ...
          <form onSubmit={onSubmit} className="lg:col-span-3 rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-card">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                <Field label="Địa chỉ cần đến chăm sóc" name="address" placeholder="Số nhà, tên đường, khu vực..." required />
              </div>
              <div className="sm:col-span-2">
...
function Field({ label, name, type = "text", placeholder, required }: { label: string; name: string; type?: string; placeholder?: string; required?: boolean; }) {
  return (
    <div>
      ...
      <input name={name} type={type} placeholder={placeholder} required={required} className="w-full rounded-xl border border-input bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition" />
    </div>
  );
}
...
function FloatingCall() {
  return (
    <a href="tel:0915205115" aria-label="Gọi tư vấn" className="fixed bottom-6 right-6 z-40 group">
...
function BackToTop() {
  ...
  if (!show) return null;
  return (
    <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Trở lên đầu trang" className="fixed bottom-24 right-6 z-40 grid h-11 w-11 place-items-center rounded-full bg-card border border-border shadow-card hover:bg-primary hover:text-primary-foreground transition animate-fade-in">

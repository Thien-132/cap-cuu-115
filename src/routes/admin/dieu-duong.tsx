    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {nurses.map((n) => (
          <div
            key={n.id}
            className="bg-card rounded-[2rem] border border-border shadow-card p-6 flex flex-col gap-4"
          >
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-lg font-display text-foreground">{n.name}</h3>
                <p className="text-sm text-primary font-medium bg-primary/10 inline-block px-2 py-0.5 rounded-md mt-1">
                  {n.role}
                </p>
              </div>
              <span
                className={`w-3 h-3 rounded-full ${n.status === "active" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-slate-300"}`}
              ></span>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-2 pt-4 border-t border-border">
              <button
                ...
                className="text-sm font-semibold text-primary hover:bg-primary/10 px-4 py-2 rounded-xl transition-colors"
              >
                Sửa
              </button>
              <button
                onClick={() => handleDelete(n.id)}
                className="text-sm font-semibold text-emergency hover:bg-emergency/10 px-4 py-2 rounded-xl transition-colors"
              >
                Xóa
              </button>
            </div>

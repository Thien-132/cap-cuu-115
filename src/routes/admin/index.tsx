...
function StatCard({ title, value, icon: Icon, colorClass, gradientClass }: any) {
  return (
    <div className="bg-card p-6 rounded-[2rem] shadow-card border border-border flex items-center gap-5 hover:-translate-y-1 transition-transform cursor-default">
      <div
        className={`h-14 w-14 rounded-2xl flex items-center justify-center text-white shadow-soft shrink-0 ${gradientClass}`}
      >
...
function Dashboard() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold font-display tracking-tight text-foreground mb-8">
        Bảng điều khiển
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-10">
            ...
            {recentReqs.length === 0 ? (
              ...
              <div className="space-y-4">
                {recentReqs.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center justify-between p-4 rounded-2xl bg-secondary/30 border border-border/50 hover:bg-secondary/50 transition-colors"
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`mt-1 h-2.5 w-2.5 rounded-full shrink-0 ${req.status === "new" ? "bg-emergency animate-pulse" : req.status === "processing" ? "bg-blue-500" : "bg-emerald-500"}`}
                      />
                      <div>
        ...
        <div className="bg-card rounded-[2rem] shadow-card border border-border p-8 flex flex-col">
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <Link
              to="/admin/quan-ly-xe"
              className="w-full py-3 rounded-xl bg-secondary text-foreground font-semibold hover:bg-secondary/80 transition-colors flex items-center justify-center gap-2"
            >
              Quản lý đội xe
            </Link>
          </div>

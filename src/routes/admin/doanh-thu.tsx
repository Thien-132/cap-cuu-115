...
function RevenuePage() {
  ...
  const [timeFilter, setTimeFilter] = useState<"all" | "today" | "month" | "year" | "custom">(
    "all",
  );
  ...
  const { totalRevenue, completedCount, chartData } = useMemo(() => {
    ...
    requests.forEach((req) => {
      if (req.status === "completed" && req.price) {
        ...
        if (timeFilter === "today") {
          ...
          include =
            reqDate.getFullYear() === cDate.getFullYear() &&
            reqDate.getMonth() === cDate.getMonth() &&
            reqDate.getDate() === cDate.getDate();
        } else if (timeFilter === "custom" && !customDate) {
    ...
    const cData = Object.keys(dailyMap)
      .sort()
      .map((k) => ({
        name: k,
        total: dailyMap[k],
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-[2rem] border border-border shadow-card p-8 flex items-center gap-6">
          ...
          <div>
            ...
            <h2 className="text-3xl font-bold font-display text-foreground mt-1">
              {completedCount}{" "}
              <span className="whitespace-nowrap text-lg font-medium text-muted-foreground lowercase tracking-normal">
                Chuyến xe
              </span>
            </h2>
          </div>

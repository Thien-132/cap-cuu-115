        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-secondary/40 text-muted-foreground border-b border-border">
              <tr>
                <th className="whitespace-nowrap px-6 py-4 font-semibold">Số điện thoại</th>
                <th className="whitespace-nowrap px-6 py-4 font-semibold text-center">
                  Hoàn thành
                </th>
                <th className="whitespace-nowrap px-6 py-4 font-semibold text-center">Đã hủy</th>
                <th className="whitespace-nowrap px-6 py-4 font-semibold text-center">Báo giả</th>
                <th className="whitespace-nowrap px-6 py-4 font-semibold text-center">
                  Trạng thái chặn
                </th>
            ...
            <tbody className="divide-y divide-border">
              {history.length === 0 ? (
                <tr>
                  <td
                    colSpan={6}
                    className="whitespace-nowrap px-6 py-12 text-center text-muted-foreground"
                  >
                ...
                history.map((record) => (
                  <tr key={record.phone} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-center">
                      <span
                        className={`px-3 py-1 rounded-full font-medium text-xs ${record.fakeReports > 0 ? "bg-red-50 text-red-600" : "bg-secondary/50 text-muted-foreground"}`}
                      >
                        {record.fakeReports}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-center">
                      {record.isBlocked ? (
                        <span className="whitespace-nowrap px-3 py-1 rounded-full bg-red-100 text-red-700 font-bold text-xs">
                          Đang bị chặn
                        </span>
                      ) : (
                        <span className="whitespace-nowrap px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 font-bold text-xs">
                          Bình thường
                        </span>
                      )}
                    </td>
